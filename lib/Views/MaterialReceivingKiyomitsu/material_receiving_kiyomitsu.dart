...
class MaterialReceivingKiyomitsuPage extends StatefulWidget {
  const MaterialReceivingKiyomitsuPage({super.key});

  @override
  MaterialReceivingKiyomitsuPageState createState() => MaterialReceivingKiyomitsuPageState();
}

class MaterialReceivingKiyomitsuPageState extends State<MaterialReceivingKiyomitsuPage> with WidgetsBindingObserver {
  ...
  Future<void> _getTableData() async {
    ...
    TableDataPara para = TableDataPara(
      plant: "DG",
      runCard: "",
      operationList: ['409', '410', '416', '419', '430', '446'],
      page: _page + 1,
      pageSize: 15,
    );
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        ...
        if (strList.length == 12) {
          form = {'OperationGroup': '清光', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[5], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 3) {
          form = {'OperationGroup': '清光', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[2], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 1) {
          form = {'OperationGroup': '清光', 'RUN_CARD': strList[0], 'QTY': 0, 'TICKET_SN': '', 'PLANT': "DG", 'INSPECTOR': accId};
        } else {
          EasyLoading.dismiss();
          if (context.mounted) {
