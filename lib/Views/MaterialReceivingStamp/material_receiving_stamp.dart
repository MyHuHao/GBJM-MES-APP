...
class MaterialReceivingStampPage extends StatefulWidget {
  const MaterialReceivingStampPage({super.key});

  @override
  MaterialReceivingStampPageState createState() => MaterialReceivingStampPageState();
}

class MaterialReceivingStampPageState extends State<MaterialReceivingStampPage> with WidgetsBindingObserver {
  ...
  Future<void> _getTableData() async {
    ...
    TableDataPara para = TableDataPara(
      plant: "DG",
      runCard: "",
      operationList: ['309', '310', '329', '336', '638'],
      page: _page + 1,
      pageSize: 15,
    );
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        ...
        if (strList.length == 12) {
          form = {'OperationGroup': '平面抛光 镭射 打钢印', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[5], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 3) {
          form = {'OperationGroup': '平面抛光 镭射 打钢印', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[2], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 1) {
          form = {'OperationGroup': '平面抛光 镭射 打钢印', 'RUN_CARD': strList[0], 'QTY': 0, 'TICKET_SN': '', 'PLANT': "DG", 'INSPECTOR': accId};
        } else {
          EasyLoading.dismiss();
          if (context.mounted) {
