...
class MaterialReceivingRoughPage extends StatefulWidget {
  const MaterialReceivingRoughPage({super.key});

  @override
  MaterialReceivingRoughPageState createState() => MaterialReceivingRoughPageState();
}

class MaterialReceivingRoughPageState extends State<MaterialReceivingRoughPage> with WidgetsBindingObserver {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        ...
        if (strList.length == 12) {
          form = {'OperationGroup': '开粗', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[5], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 3) {
          form = {'OperationGroup': '开粗', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[2], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 1) {
          form = {'OperationGroup': '开粗', 'RUN_CARD': strList[0], 'QTY': 0, 'TICKET_SN': '', 'PLANT': "DG", 'INSPECTOR': accId};
        } else {
          EasyLoading.dismiss();
          if (context.mounted) {
