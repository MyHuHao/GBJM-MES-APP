...
class MaterialReceivingDrumPage extends StatefulWidget {
  const MaterialReceivingDrumPage({super.key});

  @override
  MaterialReceivingDrumPageState createState() => MaterialReceivingDrumPageState();
}

class MaterialReceivingDrumPageState extends State<MaterialReceivingDrumPage> with WidgetsBindingObserver {
  ...
  Future<void> _getTableData() async {
    ...
    TableDataPara para = TableDataPara(
      plant: "DG",
      runCard: "",
      operationList: ['320', '321', '145', '176', '194', '303', '304', '305', '306', '332', '334', '335', '337', '339', '307', '308'],
      page: _page + 1,
      pageSize: 15,
    );
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        ...
        if (strList.length == 12) {
          form = {'OperationGroup': '滚筒', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[5], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 3) {
          form = {'OperationGroup': '滚筒', 'RUN_CARD': strList[0], 'QTY': strList[1], 'TICKET_SN': strList[2], 'PLANT': "DG", 'INSPECTOR': accId};
        } else if (strList.length == 1) {
          form = {'OperationGroup': '滚筒', 'RUN_CARD': strList[0], 'QTY': 0, 'TICKET_SN': '', 'PLANT': "DG", 'INSPECTOR': accId};
        } else {
          EasyLoading.dismiss();
          if (context.mounted) {
